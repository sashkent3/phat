...
namespace phat
{
    class swap_exhaustive_twist_reduction_v2
    {
    public:
            ...
            for (index cur_dim = boundary_matrix.get_max_dim(); cur_dim >= 1; cur_dim--)
            {
                for (index cur_col = 0; cur_col < nr_columns; cur_col++)
                {
                    if (boundary_matrix.get_dim(cur_col) == cur_dim)
                    {
                        ...
                        std::list<index> potential_swaps;
                        while (lowest_one != -1)
                        {
                            if (lowest_one_lookup[lowest_one] != -1)
                            {
                                if (!is_pivot)
                                {
                                    potential_swaps.insert(potential_swaps.end(), lowest_one);
                                }
#if COUNT_OPS
                        ...
                        if (is_pivot)
                        {
                            for (index j : potential_swaps)
                            {
                                int old_size = boundary_matrix.size(lowest_one_lookup[j]);
#if COUNT_OPS
                                count_ops += boundary_matrix.size(cur_col);
                                count_col_ops += 1;
#endif
                                boundary_matrix.add_to(cur_col, lowest_one_lookup[j]);
                                if (!(boundary_matrix.size(lowest_one_lookup[j]) < old_size))
                                {
#if COUNT_OPS
                                    count_ops += boundary_matrix.size(cur_col);
                                    count_col_ops += 1;
	    ...
	    phat::Reduction_statistics stats("Swap-exhaustive-twist-v2",count_ops,count_col_ops,count_final_ones,max_size);
	    stats.print();
#endif
        }
    };
