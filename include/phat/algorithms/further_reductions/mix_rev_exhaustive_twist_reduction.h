...
namespace phat
{
    class mix_rev_exhaustive_twist_reduction
    {
    public:
                        ...
                        while (lowest_one != -1)
                        {
                            if (lowest_one_lookup[lowest_one] != -1)
                            {
                                ...
                                { // Check if current column is sparser than pervious sparsiest one
                                    ...
                                    int cur_nb_ones = final_col_value.size() + boundary_matrix.size(cur_col);
...
#if COUNT_OPS
	    phat::Reduction_statistics stats("Mix rev exhaustive twist",count_ops,count_col_ops,count_final_ones,max_size);
	    stats.print();
